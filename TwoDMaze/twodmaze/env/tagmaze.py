...
class raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        self.metadata["name"] = "two_d_maze_obstacles"
...
class Scenario(BaseScenario):
    def make_world(self, num_good=1, num_adversaries=1, num_obstacles=3):
        world = World()
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.max_speed = 1.0 if agent.adversary else 1
        # add landmarks
        world.landmarks = [Landmark() for i in range(num_landmarks)]
        for i, landmark in enumerate(world.landmarks):
            landmark.name = "landmark %d" % i
            landmark.collide = True if i < len(world.landmarks)-1 else False
    ...
    def reset_world(self, world, np_random):
        ...
        for i, landmark in enumerate(world.landmarks):
            landmark.color = np.array([0.25, 0.25, 0.25]) if i < len(world.landmarks)-1 else np.array([0.5, 0.5, 0.5])
