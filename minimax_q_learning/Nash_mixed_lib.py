...
def test_shift_and_scale_invariance(M):
    ...
    x1 = np.maximum(x1, 0); x1 /= x1.sum() if x1.sum() > 0 else 1
    y1 = np.maximum(y1, 0); y1 /= y1.sum() if y1.sum() > 0 else 1
    ...
    x2 = np.maximum(x2, 0); x2 /= x2.sum() if x2.sum() > 0 else 1
    y2 = np.maximum(y2, 0); y2 /= y2.sum() if y2.sum() > 0 else 1
    ...
    x3 = np.maximum(x3, 0); x3 /= x3.sum() if x3.sum() > 0 else 1
    y3 = np.maximum(y3, 0); y3 /= y3.sum() if y3.sum() > 0 else 1
...
def run_all_tests(verbose=False):
    ...
    for name, M in cases:
        ...
        x = np.maximum(x, 0); sx = x.sum(); x = x/sx if sx>0 else np.full_like(x, 1/len(x))
        y = np.maximum(y, 0); sy = y.sum(); y = y/sy if sy>0 else np.full_like(y, 1/len(y))
